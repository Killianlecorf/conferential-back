...
export async function userController(fastify: FastifyInstance, options: FastifyPluginOptions) {
    ...
    fastify.get('/users/conferences/join', { preHandler: decodeJwt }, async (request, reply) => {
        const currentUser = (request as any).user;
        if (!currentUser) return reply.status(401).send();
        ...
        try {
            ...
            const joinedConfs = user.conferences.getItems().map(conf => ({
                id: conf.id,
                title: conf.title,
            ...
            await orm.close();
            return reply.status(200).send(joinedConfs);
